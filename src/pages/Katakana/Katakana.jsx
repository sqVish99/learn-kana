...
import useOptionSelect from "@/components/UseOptionSelect/UseOptionSelect"; // Import the custom hook
...
const Katakana = () => {
	...
	const generateNewPracticeSet = () => {
		const newSet = generatePracticeSet(katakanaData, PRACTICE_SET_SIZE);
	...
	useEffect(() => {
		if (currentPracticeSet.length > 0 && !practiceComplete) {
			...
			setOptions(generateOptions(currentChar, katakanaData));
		}
	...
	if (practiceComplete) {
		return (
			<PracticeComplete
				...
				message={`Great job! You've practiced ${PRACTICE_SET_SIZE} katakana characters.`}
